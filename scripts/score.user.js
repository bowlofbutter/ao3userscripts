// ==UserScript==
// @name        AO3 Quality score
// @description Uses the kudos/hits ratio, number of chapters, and statistical evaluation to score and sort AO3 works. All credits goes to Min_ and cupkax at Greasyfork  I just modified the script for personal use to insert color coded symbols and removed the background highlight.
...
const CONFIG = {
    ...
    colors: {
        red: '#BE5A86',    // background color for low scores
        yellow: '#F9BC86', // background color for medium scores
        green: '#6FDAC3'   // background color for high scores
    }
};
...
(($) => {
    ...
    if (percents >= CONFIG.thresholds.high) {
        symbol = '▲';
        symbolColor = CONFIG.colors.green;
    } else if (percents >= CONFIG.thresholds.low) {
        symbol = '◆';
        symbolColor = CONFIG.colors.yellow;
    } else {
        symbol = '▼';
        symbolColor = CONFIG.colors.red;
    }
    ...
    // Add coloured symbol
    const $symbolSpan = $('<span>').text(symbol + ' ').css('color', symbolColor);

    // Add score text (in light grey)
    const $scoreText = $('<span>').text(percents_print).css('color', '#F7F7F7');

    // Append both
    $ratioValue.append($symbolSpan).append($scoreText);
}
